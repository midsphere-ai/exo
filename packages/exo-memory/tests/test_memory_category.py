...
from exo.memory.long_term import LongTermMemory  # pyright: ignore[reportMissingImports]
...
class TestShortTermMemoryCategoryFilter:
    ...
    async def test_search_category_combined_with_query(self) -> None:
        stm = ShortTermMemory()
        await stm.add(
            HumanMemory(content="dark mode preference", category=MemoryCategory.USER_PROFILE)
        )
        await stm.add(
            HumanMemory(content="light mode preference", category=MemoryCategory.USER_PROFILE)
        )
...
class TestLongTermMemoryCategoryFilter:
    async def test_search_by_category(self) -> None:
        ...
        await ltm.add(
            MemoryItem(
                content="semantic fact", memory_type="human", category=MemoryCategory.SEMANTIC
            )
        )
        await ltm.add(
            MemoryItem(content="an episode", memory_type="human", category=MemoryCategory.EPISODIC)
        )
    ...
    async def test_search_no_category_filter_returns_all(self) -> None:
        ltm = LongTermMemory()
        await ltm.add(
            MemoryItem(content="a", memory_type="human", category=MemoryCategory.USER_PROFILE)
        )
        await ltm.add(
            MemoryItem(content="b", memory_type="human", category=MemoryCategory.SEMANTIC)
        )
    ...
    async def test_search_category_with_none_items(self) -> None:
        ...
        await ltm.add(
            MemoryItem(content="has cat", memory_type="human", category=MemoryCategory.SUMMARY)
        )
    ...
    async def test_search_category_combined_with_query(self) -> None:
        ltm = LongTermMemory()
        await ltm.add(
            MemoryItem(content="python tip", memory_type="human", category=MemoryCategory.SEMANTIC)
        )
        await ltm.add(
            MemoryItem(
                content="python episode", memory_type="human", category=MemoryCategory.EPISODIC
            )
        )
        await ltm.add(
            MemoryItem(content="rust fact", memory_type="human", category=MemoryCategory.SEMANTIC)
        )
    ...
    async def test_search_empty_category_result(self) -> None:
        ltm = LongTermMemory()
        await ltm.add(
            MemoryItem(content="hello", memory_type="human", category=MemoryCategory.CONVERSATION)
        )
