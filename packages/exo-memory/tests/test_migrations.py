...
from exo.memory.migrations import (  # noqa: E402  # pyright: ignore[reportMissingImports]
    Migration,
    MigrationError,
    MigrationRegistry,
    run_migrations,
...
class TestRunMigrationsSQLite:
    ...
    async def test_applies_single_migration(self, store: SQLiteMemoryStore) -> None:
        ...
        async def add_column(db):
            await db.execute("ALTER TABLE memory_items ADD COLUMN tags TEXT DEFAULT ''")
            executed.append(1)
...
class TestRunMigrationsEdgeCases:
    ...
    async def test_real_schema_change(self, store: SQLiteMemoryStore) -> None:
        ...
        async def add_priority(db):
            await db.execute("ALTER TABLE memory_items ADD COLUMN priority INTEGER DEFAULT 0")
