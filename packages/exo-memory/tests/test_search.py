...
from exo.memory.search import SearchManager  # pyright: ignore[reportMissingImports]
...
class TestSearchManagerSearch:
    ...
    async def test_per_store_limit_prevents_domination(self) -> None:
        ...
        big_store = MockStore(
            [
                _make_item(
                    f"big {i}", item_id=f"b{i}", created_at=f"2025-01-{i + 1:02d}T00:00:00+00:00"
                )
                for i in range(50)
            ]
        )
        small_store = MockStore(
            [
                _make_item(
                    f"small {i}", item_id=f"s{i}", created_at=f"2026-06-{i + 1:02d}T00:00:00+00:00"
                )
                for i in range(3)
            ]
        )
