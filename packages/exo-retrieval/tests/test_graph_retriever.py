...
from exo.retrieval.graph_retriever import GraphRetriever
from exo.retrieval.retriever import Retriever
from exo.retrieval.triple_extractor import Triple
...
class TestGraphRetrieverABC:
    ...
    def test_custom_parameters(self) -> None:
        gr = GraphRetriever(StubRetriever([]), [], beam_width=5, max_hops=4)
...
class TestGraphRetrieverBeamWidth:
    ...
    async def test_beam_width_limits_triples_per_entity(self) -> None:
        ...
        gr = GraphRetriever(StubRetriever(base), triples, beam_width=2, max_hops=1)
        results = await gr.retrieve("q", top_k=10)

        expanded_keys = {
            (r.chunk.document_id, r.chunk.index) for r in results if r.metadata.get("graph_hop")
        }
