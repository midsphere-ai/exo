...
from exo.retrieval.hybrid_retriever import HybridRetriever
...
class TestHybridRetrieverBasic:
    ...
    async def test_vector_only_results(self) -> None:
        ...
        hybrid = HybridRetriever(StubRetriever(vec_results), StubRetriever([]))
    ...
    async def test_sparse_only_results(self) -> None:
        ...
        hybrid = HybridRetriever(StubRetriever([]), StubRetriever(sp_results))
        results = await hybrid.retrieve("q")
        assert len(results) == 2
...
class TestHybridRetrieverFusion:
    ...
    async def test_custom_k_changes_scores(self) -> None:
        ...
        a = _result(document_id="a", index=0, content="a")
        k_small = HybridRetriever(StubRetriever([a]), StubRetriever([a]), k=10)
        k_large = HybridRetriever(StubRetriever([a]), StubRetriever([a]), k=100)
