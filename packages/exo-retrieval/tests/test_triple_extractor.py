...
from exo.retrieval.triple_extractor import Triple, TripleExtractor
...
class TestTripleExtractorExtract:
    ...
    @pytest.mark.asyncio
    async def test_extracts_triples_from_chunk(self, monkeypatch: pytest.MonkeyPatch) -> None:
        response = (
            '[{"subject": "Python", "predicate": "is", "object": "language", "confidence": 0.95}]'
        )
    ...
    @pytest.mark.asyncio
    async def test_extracts_multiple_triples(self, monkeypatch: pytest.MonkeyPatch) -> None:
        response = """[
    ...
    @pytest.mark.asyncio
    async def test_multiple_chunks(self, monkeypatch: pytest.MonkeyPatch) -> None:
        responses = [
    ...
    @pytest.mark.asyncio
    async def test_source_chunk_id_format(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @pytest.mark.asyncio
    async def test_custom_template_used(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestParseTriples:
    ...
    def test_non_dict_items_skipped(self) -> None:
        content = (
            '["not a dict", {"subject": "A", "predicate": "B", "object": "C", "confidence": 0.7}]'
        )
        triples = TripleExtractor._parse_triples(content, "doc:0")
        assert len(triples) == 1
