...
class TestQueryRewriterRewrite:
    @pytest.mark.asyncio
    async def test_rewrite_returns_llm_response(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rewrite_strips_whitespace(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rewrite_sends_query_in_prompt(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestQueryRewriterHistory:
    @pytest.mark.asyncio
    async def test_history_included_in_prompt(self, monkeypatch: pytest.MonkeyPatch) -> None:
        capturing = CapturingProvider()
        monkeypatch.setattr(
            "exo.models.get_provider",
            lambda *a, **kw: capturing,
