"""CLI entry point for Exo Search — AI-powered search engine.

Usage:
    python -m exo.search "What is quantum computing?"
    python -m exo.search --quality balanced "CRISPR gene editing"
    python -m exo.search --stream "latest AI news"
    python -m exo.search --chat
    python -m exo.search --serve
"""
...
async def run_search(
    ...
    if stream:
        ...
        print(f"\nSearching ({quality} mode)...\n")
        result = await search_with_details(query, mode=quality, chat_history=history, config=config)
        print(result.answer)
...
async def run_chat(
    ...
    print("Exo Search Interactive Chat")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="exo-search",
        description="Exo Search - AI-Powered Search Engine with Citations",
    )
    ...
    parser.add_argument("--stream", "-s", action="store_true", help="Show progress in real-time")
    parser.add_argument("--chat", "-c", action="store_true", help="Interactive multi-turn chat")
