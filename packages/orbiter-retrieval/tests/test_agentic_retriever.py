...
from orbiter.retrieval.agentic_retriever import AgenticRetriever
...
class TestAgenticRetrieverConfig:
    ...
    def test_defaults(self) -> None:
        ar = AgenticRetriever(
            MockRetriever([[]]), QueryRewriter("openai:gpt-4o"), "openai:gpt-4o"
        )
...
class TestAgenticRetrieverRetrieve:
    @pytest.mark.asyncio
    async def test_stops_on_sufficient_first_round(
        ...
        monkeypatch.setattr(
            "orbiter.models.get_provider", lambda *a, **kw: provider
        )
    ...
    @pytest.mark.asyncio
    async def test_retries_on_insufficient_results(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    async def test_max_rounds_limit(
        ...
        rewriter = QueryRewriter("openai:gpt-4o")
        ar = AgenticRetriever(
            base, rewriter, "openai:gpt-4o", max_rounds=2
        )
        out = await ar.retrieve("test query")
    ...
    @pytest.mark.asyncio
    async def test_empty_retrieval_no_judge_call(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_custom_sufficiency_threshold(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        rewriter = QueryRewriter("openai:gpt-4o")
        ar = AgenticRetriever(
            base, rewriter, "openai:gpt-4o", sufficiency_threshold=0.4
        )
        out = await ar.retrieve("test")
