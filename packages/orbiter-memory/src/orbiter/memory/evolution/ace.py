...
class Counters:
    ...
    __slots__ = ("helpful", "harmful", "neutral")

    def __init__(
        self, helpful: int = 0, harmful: int = 0, neutral: int = 0
    ) -> None:
...
class ACEStrategy(MemoryEvolutionStrategy):
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self._counters = {
                mid: Counters.from_dict(data) for mid, data in raw.items()
            }
        ...
        for item in items:
            if model is not None:
                prompt = _REFLECT_PROMPT.format(
                    content=item.content, feedback=feedback
                )
