...
class TestEvolveWithoutModel:
    async def test_returns_items_without_model(
    ...
    async def test_context_without_model_key(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ReMeStrategy()
...
class TestEvolveWithModel:
    async def test_extracts_patterns_via_model(
        self, sample_items: list[MemoryItem]
    ) -> None:
        model = MockProvider(
            patterns=[
    ...
    async def test_model_returns_non_array(
        self, sample_items: list[MemoryItem]
    ) -> None:
    ...
    async def test_model_returns_empty_array(
        self, sample_items: list[MemoryItem]
    ) -> None:
...
class TestExtractPatterns:
    ...
    async def test_handles_non_dict_entries(self) -> None:
        model = MockProvider(raw='[{"content": "valid", "pattern_type": "success", "when_to_use": "always"}, "not a dict", 42]')
        items = [MemoryItem(content="source", memory_type="test", id="s1")]
        strategy = ReMeStrategy()
        result = await strategy.extract_patterns(items, model)
        assert len(result) == 1
