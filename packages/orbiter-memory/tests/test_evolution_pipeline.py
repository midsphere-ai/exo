...
class SuffixStrategy(MemoryEvolutionStrategy):
    ...
    async def evolve(
        ...
        *,
        ...
        return [
            item.model_copy(update={"content": item.content + self._suffix})
            for item in items
        ]
...
class DuplicateStrategy(MemoryEvolutionStrategy):
    ...
    async def evolve(
        ...
        *,
        ...
        return [
            item.model_copy(update={"content": self._prefix + item.content})
            for item in items
        ]
...
class ContextAwareStrategy(MemoryEvolutionStrategy):
    ...
    async def evolve(
        ...
        *,
        ...
        return [
            item.model_copy(update={"content": f"[{tag}] {item.content}"})
            for item in items
        ]
...
class TestSequentialComposition:
    ...
    async def test_sequential_filter_then_transform(
...
class TestParallelComposition:
    ...
    async def test_parallel_merge_last_write_wins(
        self, sample_items: list[MemoryItem]
    ) -> None:
    ...
    async def test_parallel_with_filter_merges(
        self, sample_items: list[MemoryItem]
    ) -> None:
    ...
    async def test_parallel_context_forwarded(
        self, sample_items: list[MemoryItem]
    ) -> None:
...
class TestEdgeCases:
    ...
    async def test_item_ids_preserved_sequential(
        self, sample_items: list[MemoryItem]
    ) -> None:
        pipeline = UpperCaseStrategy() >> SuffixStrategy("!")
        result = await pipeline.evolve(sample_items)
    ...
    async def test_original_items_not_mutated(
        self, sample_items: list[MemoryItem]
    ) -> None:
        pipeline = UpperCaseStrategy() >> SuffixStrategy("!")
