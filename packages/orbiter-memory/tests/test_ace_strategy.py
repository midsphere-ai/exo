...
class TestEvolve:
    async def test_keeps_all_without_feedback(
    ...
    async def test_prunes_high_harmful_ratio(
        self, sample_items: list[MemoryItem]
    ) -> None:
    ...
    async def test_keeps_at_threshold(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy(harmful_threshold=0.5)
...
class TestReflect:
    async def test_reflect_without_model(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy()
    ...
    async def test_reflect_with_model(
        self, sample_items: list[MemoryItem]
    ) -> None:
    ...
    async def test_reflect_with_mapping(
        self, sample_items: list[MemoryItem]
    ) -> None:
        model = MockModel(
            label="neutral",
    ...
    async def test_reflect_invalid_model_output(
        self, sample_items: list[MemoryItem]
    ) -> None:
...
class TestCurate:
    async def test_curate_keeps_high_score(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy()
        for _ in range(10):
    ...
    async def test_curate_removes_low_score(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy()
        for _ in range(10):
    ...
    async def test_curate_keeps_no_feedback(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy()
    ...
    async def test_curate_custom_threshold(
        self, sample_items: list[MemoryItem]
    ) -> None:
        strategy = ACEStrategy()
