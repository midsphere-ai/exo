...
from orbiter._internal.workflow_checkpoint import (
    WorkflowCheckpoint,
...
class TestSwarmResume:
    ...
    async def test_resume_with_checkpoint_enabled(self) -> None:
        ...
        result = await swarm.resume(
            old_store, provider=provider, checkpoint=new_store,
        )
...
class TestCheckpointPublicAPI:
    """Verify checkpoint types are importable from orbiter."""

    def test_checkpoint_importable(self) -> None:
        from orbiter import WorkflowCheckpoint as WorkflowCheckpointImport

        assert WorkflowCheckpointImport is WorkflowCheckpoint

    def test_checkpoint_store_importable(self) -> None:
        from orbiter import WorkflowCheckpointStore as WorkflowCheckpointStoreImport
