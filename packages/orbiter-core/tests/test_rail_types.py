"""Tests for orbiter.rail_types — typed event input models for rails."""

import pytest
from pydantic import ValidationError

from orbiter.hooks import HookPoint
from orbiter.rail_types import (
    InvokeInputs,
    ModelCallInputs,
    RailContext,
    ToolCallInputs,
...
class TestToolCallInputs:
    ...
    def test_full_creation(self) -> None:
        inputs = ToolCallInputs(
            tool_name="search",
            arguments={"query": "orbiter docs"},
            result="found 3 results",
            metadata={"duration_ms": 42},
        )
        assert inputs.arguments == {"query": "orbiter docs"}
