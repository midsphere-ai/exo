...
class TestScheduleBasic:
    @pytest.mark.asyncio()
    async def test_schedule_runs_submitted_tasks(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
    ...
    @pytest.mark.asyncio()
    async def test_schedule_skips_non_submitted(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
        t1 = mgr.create("task-1")
    ...
    @pytest.mark.asyncio()
    async def test_schedule_marks_failed_on_exception(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
...
class TestResume:
    ...
    @pytest.mark.asyncio()
    async def test_resumed_task_picked_up_by_schedule(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
        t = mgr.create("task")
        mgr.update(t.id, status=TaskStatus.WORKING)
        mgr.update(t.id, status=TaskStatus.PAUSED)
        scheduler.resume(t.id)
...
class TestCancel:
    ...
    def test_cancel_already_completed_raises(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
        t = mgr.create("task")
        mgr.update(t.id, status=TaskStatus.WORKING)
        mgr.update(t.id, status=TaskStatus.COMPLETED)
    ...
    @pytest.mark.asyncio()
    async def test_cancel_during_schedule_skips_execution(self, scheduler: TaskScheduler, mgr: TaskManager) -> None:
        t = mgr.create("task")
        executed: list[str] = []
