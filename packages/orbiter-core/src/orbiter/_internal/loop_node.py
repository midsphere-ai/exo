...
class LoopNode:
    ...
    def __init__(
        ...
        *,
        ...
        if modes == 0:
            raise LoopError(
                "LoopNode requires exactly one of: count, items, or condition"
            )
        if modes > 1:
            raise LoopError(
                "LoopNode requires exactly one of: count, items, or condition"
    ...
    def _resolve_iterations(self, state: dict[str, Any]) -> int | None:
        ...
        if self.items is not None:
            ...
            if arr is None:
                raise LoopError(
                    f"Loop '{self.name}': items key '{self.items}' not found in state"
                )
            if not isinstance(arr, (list, tuple)):
                raise LoopError(
                    f"Loop '{self.name}': items key '{self.items}' is not a list"
                )
            return len(arr)
    ...
    def _check_condition(self, state: dict[str, Any]) -> bool:
        ...
        try:
            ...
            raise LoopError(
                f"Loop '{self.name}' condition evaluation failed: {exc}"
            ) from exc
    ...
    def describe(self) -> dict[str, Any]:
        ...
        mode = (
            "count" if self.count is not None
            else "items" if self.items is not None
            else "condition"
        )
        desc: dict[str, Any] = {
            "type": "loop",
