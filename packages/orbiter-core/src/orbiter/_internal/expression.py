...
_SAFE_NAMES = frozenset({
    "True", "False", "None", "int", "float", "str", "bool", "len", "abs",
    "min", "max", "round", "list", "dict", "tuple", "set", "isinstance", "type",
})
_BLOCKED_ATTRS = frozenset({
    "__class__", "__bases__", "__subclasses__", "__mro__", "__globals__",
    "__code__", "__func__", "__self__", "__dict__", "__init__", "__new__",
    "__del__", "__import__", "__builtins__", "__loader__", "__spec__",
})
_CMP = {
...
_UNA = {ast.UAdd: operator.pos, ast.USub: operator.neg, ast.Not: operator.not_, ast.Invert: operator.invert}
...
def _ev(node: ast.AST, v: dict[str, Any]) -> Any:
    ...
    if isinstance(node, ast.Name):
        ...
        if node.id in _SAFE_NAMES:
            return __builtins__[node.id] if isinstance(__builtins__, dict) else getattr(__builtins__, node.id)  # type: ignore[index]
    ...
    if isinstance(node, ast.JoinedStr):
        parts = []
        for val in node.values:
            parts.append(str(_ev(val.value, v)) if isinstance(val, ast.FormattedValue) else str(_ev(val, v)))
        return "".join(parts)
