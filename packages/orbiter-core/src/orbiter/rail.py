...
def _build_inputs(
    ...
    """
    from orbiter.rail_types import InvokeInputs, ModelCallInputs, ToolCallInputs
...
class RailManager:
    ...
    :class:`~orbiter.hooks.HookManager` via :meth:`hook_for`.
    ...
    async def run(self, event: HookPoint, **data: Any) -> RailAction:
        ...
        Builds a :class:`~orbiter.rail_types.RailContext` from the keyword
    ...
    def hook_for(self, event: HookPoint) -> Hook:
        ...
        :class:`~orbiter.hooks.HookManager`.  If any rail returns
        ...
        async def _hook(**hook_data: Any) -> None:
            ...
            if action == RailAction.ABORT:
                raise RailAbortError(
                    "RailManager", reason=f"Rail aborted at {event.value}"
                )
