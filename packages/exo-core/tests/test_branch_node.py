"""Tests for exo._internal.branch_node — conditional routing in workflows."""
...
from exo._internal.branch_node import BranchError, BranchNode
...
class TestBranchNodeConstruction:
    ...
    def test_describe_string_condition(self) -> None:
        ...
        branch = BranchNode(name="check", condition="x > 5", if_true="a", if_false="b")
    ...
    def test_repr_callable_condition(self) -> None:
        ...
        branch = BranchNode(name="check", condition=lambda s: True, if_true="a", if_false="b")
        r = repr(branch)
...
class TestBranchNodeEvaluateString:
    ...
    def test_invalid_expression_raises(self) -> None:
        ...
        branch = BranchNode(name="check", condition="import os", if_true="a", if_false="b")
    ...
    def test_undefined_variable_raises(self) -> None:
        ...
        branch = BranchNode(name="check", condition="missing_var > 5", if_true="a", if_false="b")
        with pytest.raises(BranchError, match="condition evaluation failed"):
            branch.evaluate({})
...
class TestBranchNodeEvaluateCallable:
    ...
    def test_callable_receives_state(self) -> None:
        ...
        branch = BranchNode(name="check", condition=capture, if_true="a", if_false="b")
    ...
    def test_callable_exception_raises_branch_error(self) -> None:
        ...
        branch = BranchNode(name="check", condition=bad_fn, if_true="a", if_false="b")
...
class TestBranchNodeStreamWorkflow:
    ...
    async def test_stream_branch_emits_status(self) -> None:
        ...
        from exo.types import StatusEvent
        ...
        events = []
        async for event in run.stream(swarm, "test input", provider=provider, detailed=True):
            events.append(event)
...
class TestBranchNodeRegression:
    ...
    def test_branch_node_not_treated_as_group(self) -> None:
        ...
        branch = BranchNode(name="check", condition="True", if_true="a", if_false="b")
