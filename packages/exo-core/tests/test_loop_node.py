"""Tests for exo._internal.loop_node — iteration in workflows."""
...
from exo._internal.loop_node import BREAK_SENTINEL, LoopError, LoopNode
...
class TestLoopNodeCountMode:
    ...
    async def test_multiple_body_agents(self) -> None:
        ...
        provider = _make_provider(
            [
                _output("a0"),
                _output("b0"),  # iteration 0
                _output("a1"),
                _output("b1"),  # iteration 1
            ]
        )
...
class TestLoopNodeBreak:
    ...
    async def test_break_terminates_loop_early(self) -> None:
        ...
        async def break_complete(messages: Any, **kwargs: Any) -> Any:
            nonlocal call_count
            call_count += 1
            text = "result [BREAK]" if call_count >= 2 else f"iter_{call_count}"
            resp = _output(text)
