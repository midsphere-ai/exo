...
class BranchNode:
    ...
    def evaluate(self, state: dict[str, Any]) -> str:
        ...
        try:
            ...
            raise BranchError(f"Branch '{self.name}' condition evaluation failed: {exc}") from exc
        except Exception as exc:
            raise BranchError(f"Branch '{self.name}' condition raised an error: {exc}") from exc
