...
class TestLLMCallOperator:
    ...
    def test_get_state(self) -> None:
        op = LLMCallOperator(
            "llm-1", _fake_llm, system_prompt="a", user_prompt="b"
        )
    ...
    def test_state_roundtrip(self) -> None:
        op = LLMCallOperator(
            "llm-1", _fake_llm, system_prompt="orig-sys", user_prompt="orig-usr"
        )
...
class TestToolCallOperator:
    ...
    def test_get_tunables(self) -> None:
        op = ToolCallOperator(
            "tool-1", _fake_tool, tool_description="Searches the web"
        )
        tunables = op.get_tunables()
...
class TestMemoryCallOperator:
    ...
    def test_trace_to_dict(self) -> None:
        t = MemoryCallTrace(
            operator_name="m", enabled=True, max_retries=1, kwargs={"k": "v"}
        )
        d = t.to_dict()
