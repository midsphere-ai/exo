...
Define an evidence-backed path for Orbiter's long-context work. The decision for this story is that Orbiter should stage "infinite context" as provider-agnostic, branch-scoped summaries, checkpoints, retrieval, and artifact offloading. It should not promise unlimited raw-history replay or depend on opaque vendor state as its source of truth.

## Current Inventory And Exact Touchpoints

| Surface | Current touchpoints | What exists today |
| --- | --- | --- |
| Core agent prompt assembly | `packages/orbiter-core/src/orbiter/agent.py` in `_run_inner()`, `_apply_context_windowing()`, `_inject_long_term_knowledge()`, `_offload_large_result()`, `branch()`, and `_make_spawn_self_tool()` | The real runtime path already does history loading, transient summarization, trimming, vector injection, branch copy, and large-result offload. |
| Streaming visibility | `packages/orbiter-core/src/orbiter/runner.py`, `packages/orbiter-core/src/orbiter/types.py` (`ContextEvent`) | Streamed runs emit context actions today, but only for the transient helper path. |
| Short-term persistence | `packages/orbiter-memory/src/orbiter/memory/persistence.py`, `base.py`, `short_term.py` | Human, AI, and tool results are persisted and replayed into later runs. |
| Long-term / vector search | `packages/orbiter-memory/src/orbiter/memory/long_term.py`, `backends/vector.py` | Orbiter has keyword long-term memory plus in-memory and Chroma-backed vector search. |
| Context primitives | `packages/orbiter-context/src/orbiter/context/context.py`, `state.py`, `checkpoint.py`, `token_tracker.py` | Branchable state, in-memory checkpoints, and token tracking already exist as standalone primitives. |
| Prompt-building / processors / workspace | `packages/orbiter-context/src/orbiter/context/prompt_builder.py`, `processor.py`, `workspace.py`, `_internal/knowledge.py`, `tools.py`, `neuron.py` | Orbiter has a richer context engine package, but the main `Agent.run()` path does not use it end-to-end. |
| Web parallel implementations | `packages/orbiter-web/src/orbiter_web/services/memory.py`, `routes/playground.py`, `routes/checkpoints.py`, `routes/context_state.py` | Orbiter Web has its own memory summary/checkpoint concepts, separate from core runtime context objects. |
| Evidence tests | `tests/integration/test_context_summarization.py`, `tests/integration/test_context_vector_injection.py`, `tests/integration/test_branching_isolation.py`, `tests/integration/test_spawn_memory_isolation.py`, `packages/orbiter-context/tests/test_context_integration.py` | These prove the current behavior and current limits more reliably than the guides alone. |

## What Orbiter Does Today
...
- Summarization reuses `orbiter-memory`'s `generate_summary()` and injects a transient `SystemMessage` with `[Conversation Summary]`.
...
- Core checkpoints are also incomplete for branch continuation. `Context.snapshot()` writes only to the in-memory `CheckpointStore` owned by that `Context` instance, while Orbiter Web stores separate checkpoint rows under `workflow_runs`.
- `packages/orbiter-web/src/orbiter_web/routes/context_state.py` still returns a placeholder tree, so there is no live branch/context inspector wired to the runtime path.

### 4. Memory Integration

- If the caller does not pass `memory=...`, `Agent` auto-creates `AgentMemory(short_term=ShortTermMemory(), long_term=default_store)` when `orbiter-memory` is importable.
...
- Orbiter Web does not use that same pipeline. `packages/orbiter-web/src/orbiter_web/services/memory.py` has separate `conversation`, `sliding_window`, and `summary` strategies backed by its own SQLite tables.
...
- `_execute_tools()` offloads large string tool results when `tool.large_output` is set or the string exceeds `ORBITER_LARGE_OUTPUT_THRESHOLD`.
...
- `PromptBuilder`, neurons, and `ProcessorPipeline` are implemented and tested in `orbiter-context`.
- The main agent runtime still bypasses them in favor of bespoke helpers in `orbiter-core/src/orbiter/agent.py`.
...
| [Google Gemini long context](https://ai.google.dev/gemini-api/docs/long-context) and [context caching](https://ai.google.dev/gemini-api/docs/context-caching) | Very large input windows postpone compaction pressure, while cached prefixes reduce repeated costs. | Bigger context windows change thresholds, not architecture. Orbiter still needs persisted summaries/checkpoints because long sessions, branches, and resumed runs outlive a single prompt window. |
| [MemGPT](https://arxiv.org/abs/2310.08560) | OS-like virtual memory for LLM agents with a working context and external memory. | The memory hierarchy is relevant, but autonomous memory paging is too large a jump for Orbiter's first staged implementation. |
| [LongMem](https://arxiv.org/abs/2306.07174) | Retrieval from an external long-term memory bank instead of replaying the whole sequence. | Orbiter should treat summaries and checkpoints as searchable memory objects, not just one latest summary blob. |
| [LongLLMLingua](https://arxiv.org/abs/2310.06839) | Learned prompt compression to shrink long prompts while preserving salient content. | Compression is a possible later optimization, but only after Orbiter has persisted artifacts and regression coverage. |
| [LoCoMo](https://arxiv.org/abs/2402.17753) | A long-conversation benchmark showing that long context alone does not guarantee reliable long-term recall. | Orbiter should define bounded stage goals and regression tests instead of marketing "infinite context" as solved. |

## Decision

Orbiter should stage context management around explicit, branch-scoped compaction artifacts:
...
This is the next implementation stage Orbiter should take.
...
After stage 1 exists, Orbiter should unify branch continuation and artifact retrieval.
...
- Orbiter Web checkpoint APIs and context-state inspection should read the same underlying branch-scoped artifact model as the core runtime.
...
Only after persisted artifacts and branch scopes exist should Orbiter add more aggressive compaction controls.
...
- Evaluate the combined behavior with long-conversation benchmarks and Orbiter integration tests before attempting more autonomous memory policies.

Why this stays third:

- Compression before persistence is hard to debug and easy to regress.
- Learned or vendor-native compaction methods become safer once Orbiter already has auditable summaries/checkpoints to compare against.
...
4. A tool returns a 30 KB report. Orbiter writes artifact `A14` to persisted workspace storage, stores a short retrieval summary for it, and keeps only a pointer in the raw turn log.
...
- "True infinite context" where Orbiter guarantees perfect recall of every historical raw token forever.
- Vendor-specific opaque compacted state blobs as Orbiter's only durable source of truth.
